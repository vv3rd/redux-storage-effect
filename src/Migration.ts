...
export type VersioningConfig<TState extends Real> = Pick<BuildResult<TState>, 'migrate' | 'version'>;
...
export const buildMigration = <TLatest extends Real>(migrationsDefinition: VersioningConfigBuilder<TLatest>) => {
...
class OlderVersionBuildStep<TLatestShape extends Real, TNewerShape extends Real> extends BuildResult<TLatestShape> {
    olderVersion<TOwnShape extends Real>(
        ...
        return new OlderVersionBuildStep<TLatestShape, TOwnShape>(this.migrations);
    }
}

class CurrentVersionBuildStep<TLatest extends Real> extends BuildResult<TLatest> {
    currentVersion(this: CurrentVersionBuildStep<TLatest>, validate: (state: unknown) => TLatest) {
...
export class MigrationError extends Error {
    ...
    constructor(cause: unknown, meta: {fromVersion: number; toVersion: number; persistedData: unknown}) {
...
const describeShape = (thing: unknown): TypeDescription | ShapeDescription => {
    ...
    return Object.fromEntries(Object.entries(thing).map(([key, value]) => [key, describeShape(value)]));
};
